      ...
      <div>
          <Slide autoplay canSwipe pauseOnHover infinite transitionDuration={360}>
              {slideImages.map((slideImage, index) => (
                  <div key={index} className="flex justify-center items-center overflow-x-auto w-full px-16 py-4
                  scrollbar-none scroll-smooth">
                      <img src={slideImage.url} className="h-[310px] object-cover
        object-center rounded-md hover:border-[4px]
        border-gray-400 transition-all duration-100 ease-in" alt={slideImage.caption} />
                  </div>
              ))}
          </Slide>
      </div>
